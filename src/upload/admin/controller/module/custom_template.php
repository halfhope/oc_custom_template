...
class ControllerModuleCustomTemplate extends Controller
{
	...
	public function index()
	{
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->response->redirect($this->url->link('module/custom_template', 'token=' . $this->session->data['token'], 'SSL'));
		}
	...
	public function check_file()
	{
		...
		if (isset($this->request->post['path']) && $this->validate()) {
			...
			if (!empty($theme)) {
				$theme_info = $this->model_setting_setting->getSetting($theme, 0);
				$directory = $theme_info[$theme . '_directory'];
